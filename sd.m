function sd(varargin)
    ...
    if ~ispref(prefGroup, bookmarksName)
        ...
        % Validate the fetched bookmark
        if ~isa(bookmarks,"dictionary")
            warning("Something went wrong, and the bookmarks are being reinitialized.")
            bookmarks = initBookmarks(prefGroup, bookmarksName);
        end
    end
    ...
    if ~ispref(prefGroup, historyName)
        ...
        if ~isa(history,"struct")
            warning("Something went wrong, and the history are being reinitialized.")
            history = initHistory(prefGroup, historyName);
        end
    end
    ...
    try
                ...
                switch arg
                    case {"back", "b", "-"}
                        if history(1).cursor > 1
                            history = jump2directory(history,history(history(1).cursor - 1).source, CursorIncrement = -1);
                        end
                        navHandled = true;
                    case {"fwd", "f"}
                        if history(1).cursor < numel(history)
                            history = jump2directory(history,history(history(1).cursor + 1).source);
                        end
                        navHandled = true;
                    case "book"
        ...
        elseif nargin == 3 && strcmp(varargin{1}, "hist") && strcmp(varargin{2}, "go")
            ...
            history(1).cursor = str2double(varargin{3});
...
function history = jump2directory(history, target, options)
    arguments
        history {mustBeNonempty}
        target
        options.CursorIncrement = 1
    end
    ...
    history(1).cursor = history(1).cursor + options.CursorIncrement;
end
